...
- **Symbols / source links.** The pack produces the main nupkg plus a legacy `.symbols.nupkg` (per-TFM PDBs; `IncludeSymbols=true` from the upstream packaging). The fork does not canonicalize or gate SourceLink at the source — the narrow-divergence rule forbids changing the upstream build files (`UPSTREAM.md` divergence 13) — so the symbols PDBs carry the SDK's auto-imported document map, keyed by the absolute checkout path and non-deterministic across checkouts, and there is no fork-side CI-pack SourceLink validation (unlike the core's canonical `-p:EnableSourceLink=true` CI-pack variant). The fork's map targets `github.com/alefranz/Extensions` and each package's project URL targets `github.com/alefranz/WantsACracker-Development`; neither is yet anonymously resolvable — an open pre-publication item.
...
- **SBOM:** one SPDX 2.2 SBOM per package, checked in under `eng/sbom/`, generated with the pinned `Microsoft.Sbom.DotNetTool 4.1.5` by `eng/sbom/generate-sboms.sh` (fixed generation timestamp; the SBOMs describe the revision-pinned build variant — the git revision is stamped as the all-zeros placeholder so the checked-in SBOMs reproduce from any commit, while every product file stays identical to a live pack's except in those revision-metadata fields (the nuspec `<repository commit>` and the DLLs' `OriginalRepoCommitHash`); the canonical reproducibility check, the documented non-deterministic fields, and the revision-pinning rationale are in the script header).
