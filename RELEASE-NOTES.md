...
- **SBOM:** one SPDX 2.2 SBOM per package, checked in under `eng/sbom/`, generated with the pinned `Microsoft.Sbom.DotNetTool 4.1.5` by `eng/sbom/generate-sboms.sh` (fixed generation timestamp; the SBOMs describe the revision-pinned build variant — the git revision is stamped as the all-zeros placeholder so the checked-in SBOMs reproduce from any commit, while every product file stays byte-identical to a live pack; the canonical reproducibility check, the documented non-deterministic fields, and the revision-pinning rationale are in the script header).
